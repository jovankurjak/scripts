...
def parse_all_files(list_of_files):
    ...
    # Parse MMX
    print('MMX file: {0}'.format(list_of_files['MMX']))

    dict_startup = {
        'MMX_STARTUP': [r".startup>|Kernel Dumper information"]
    }
    dict_cpu_memory = {
        'MMX_LOW_MEM': [r"out-of-memory|crashed.*OutOfMemory|JavaCOMM detected|checkFreeMem:|low memory|memmonit_free_mem_lt"],
        'MMX_HIGH_CPU': [r"HB:.*CPU.(100|9[5-9])"]
    }
    dict_reset_shutdown = {
        'MMX_RESET': [r"SYSTEM RESET REQUESTED: RESET: reason|out-of-memory reset|requestSystemReset|SYSTEM.*RESET|RESET.*System|ErrorHandler.*reset|ooc.*(SHUTDOWN|FAST).*(FAST|SUTDOWN)|rebootSystem|watchdog.*reset"],
    ...
    dict_errors_status = {
        'MMX_ERROS': [r"error dump|Out of interrupt|can't.*mount|Check Failed|mmu fault|preempt|Created log file|free malloc object"],
    ...
    goThroughFile(list_of_files['MMX'], dict_mmx)

    # Parse A15
    print('A15 file: {0}'.format(list_of_files['A15']))
    ...
    goThroughFile(list_of_files['A15'], dict_a15)

    # Parse M4
    print('M4 file: {0}'.format(list_of_files['M4']))
    dict_m4 = {
        'M4': {
    ...
    if list_of_files.__len__() < 4:
        return

    goThroughFile(list_of_files['M4'], dict_m4)

    # Parse SubCpu
    print('SubCpu file: {0}'.format(list_of_files['SubCpu']))
    dict_subcpu = {
        'SubCpu': {
    ...
    goThroughFile(list_of_files['SubCpu'], dict_subcpu)
